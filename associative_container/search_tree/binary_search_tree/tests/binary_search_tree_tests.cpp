...
TEST(binarySearchTreePositiveTests, test1111)
{
    ...
    auto bst = std::make_unique<binary_search_tree<int, std::string>>(key_comparer(), allocator.get(), logger);
...
TEST(binarySearchTreePositiveTests, test2)
{
	...
	auto allocator = std::make_unique<allocator_sorted_list>(10000);

	auto bst = std::make_unique<binary_search_tree<int, int>>(key_comparer(), allocator.get(), logger);
...
TEST(binarySearchTreePositiveTests, test3)
{
	...
	auto bst = std::make_unique<binary_search_tree<std::string, int>>(key_comparer(), allocator.get(), logger);
...
TEST(binarySearchTreePositiveTests, test5)
{
	...
	auto bst1 = std::make_unique<binary_search_tree<int, std::string>>(key_comparer(), allocator.get(), logger);
...
TEST(binarySearchTreePositiveTests, test6)
{
	...
	auto allocator = std::make_unique<allocator_boundary_tags>(1000);
